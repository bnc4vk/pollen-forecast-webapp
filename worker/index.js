...
const openMeteoProviderMemoryCache = new Map();
const openMeteoProviderPending = new Map();
const googleProviderMemoryCache = new Map();
...
let regionalForecastCache = null;
...
function cachedOpenMeteo(lat, lng) {
  ...
  return cachedProvider(openMeteoProviderMemoryCache, openMeteoProviderPending, cacheKey, 30 * 60 * 1000, () =>
    fetchOpenMeteo(lat, lng),
  );
}

function cachedGooglePollen(lat, lng, env) {
  const cacheKey = `google:${roundCoord(lat)}:${roundCoord(lng)}`;
  return cachedProvider(googleProviderMemoryCache, googleProviderPending, cacheKey, 4 * 60 * 60 * 1000, () =>
...
async function regionalForecastContext(region, env) {
  ...
  if (!regionalForecastContextPending.has(region.id)) {
    regionalForecastContextPending.set(
      region.id,
      buildForecast(region.lat, region.lng, env)
        .then((forecast) => {
...
async function buildSpatialCellForecast(cell, tier, env) {
  ...
  const regional = await regionalForecastContext(region, env);
  ...
  if (tier === '11km') {
    const settled = await Promise.allSettled([
      ...
      cachedGooglePollen(cell.lat, cell.lng, env),
    ]);
    ...
    const settled = await Promise.allSettled([
      ...
      cachedGooglePollen(parent.lat, parent.lng, env),
    ]);
    directProviders = settled.map((result, index) =>
      result.status === 'fulfilled'
        ? result.value
...
async function buildSpatialForecast(bounds, zoom, env) {
  const tier = spatialTierForZoom(zoom);
  if (tier === 'regional') throw new Error('Regional zoom uses the Met Office region ensemble.');
  const geojson = await fetchMetOfficePolygons();
  const cells = buildSpatialCells(bounds, SPATIAL_SCALE_CONFIG[tier].cellKm, geojson);
  const forecasts = await mapWithConcurrency(cells, 8, (cell) =>
    buildSpatialCellForecast(cell, tier, env),
  );
  return {
...
async function buildForecast(lat, lng, env) {
  const settled = await Promise.allSettled([
    ...
    cachedGooglePollen(lat, lng, env),
...
async function fetchOpenMeteoGrid(bounds, category, horizonHours = MAX_TIMELAPSE_HOURS, env, { coverage = 'bounds' } = {}) {
  ...
  const regionalGeojson = coverage === 'regional' ? await fetchMetOfficePolygons(env) : null;
  const cells = regionalGeojson
    ? buildForecastCells(bounds).filter((cell) => regionForPoint(regionalGeojson, cell.lat, cell.lng))
    : buildForecastCells(bounds);
  const regionalSampled = Boolean(regionalGeojson && cells.length > MAX_OPEN_METEO_FORECAST_POINTS);
  const sampled = !regionalGeojson && cells.length > MAX_OPEN_METEO_FORECAST_POINTS;
  const sampleGrid = sampled || regionalSampled ? buildForecastSamplePoints(bounds) : { points: cells, rows: null, cols: null };
  ...
  const sampleIndexForCell = (cell) => {
    if (!sampleGrid.rows || !sampleGrid.cols) return 0;
    const latSpan = Math.max(bounds.north - bounds.south, 0.0001);
    const lngSpan = Math.max(bounds.east - bounds.west, 0.0001);
    const row = clamp(Math.round(((cell.lat - bounds.south) / latSpan) * (sampleGrid.rows - 1)), 0, sampleGrid.rows - 1);
    const col = clamp(Math.round(((cell.lng - bounds.west) / lngSpan) * (sampleGrid.cols - 1)), 0, sampleGrid.cols - 1);
    return row * sampleGrid.cols + col;
  };

  const frames = Array.from({ length: safeHorizon + 1 }, (_, offsetHours) => {
    const requestGridPoints = payloads.map((payload, index) => {
    ...
    const gridPoints = regionalSampled
      ? cells.map((cell) => {
          const samplePoint = requestGridPoints[sampleIndexForCell(cell)] || requestGridPoints[0];
          return {
            id: cell.id,
            lat: cell.lat,
            lng: cell.lng,
            bounds: cell.bounds,
            value: Number((samplePoint?.value || 0).toFixed(2)),
            score: Number((samplePoint?.score || 0).toFixed(1)),
          };
        })
      : requestGridPoints;
...
async function buildRegionalForecast(env) {
  await fetchMetOfficePage();
  const geojson = await fetchMetOfficePolygons();
  const forecasts = await Promise.all(
    MET_OFFICE_REGIONS.map(async (region) => {
      const forecast = await buildForecast(region.lat, region.lng, env);
      regionalForecastContexts.set(region.id, forecast);
      const scores = Object.fromEntries(
...
async function routeRegions(request, env) {
  if (regionalForecastCache && Date.now() - regionalForecastCache.created < 4 * 60 * 60 * 1000) {
    return jsonResponse(request, env, { ...regionalForecastCache.value, cached: true });
  }
  const value = await buildRegionalForecast(env);
  regionalForecastCache = { created: Date.now(), value };
  return jsonResponse(request, env, value);
}

async function routeSpatial(request, env) {
  const url = new URL(request.url);
  const bounds = {
  ...
  const zoom = clamp(parseCoord(url.searchParams.get('zoom'), 5), 0, 20);
  ...
  try {
    ...
    let pending = spatialForecastPending.get(requestKey);
    if (!pending) {
      pending = buildSpatialForecast(bounds, zoom, env).finally(() => {
        spatialForecastPending.delete(requestKey);
      });
      spatialForecastPending.set(requestKey, pending);
    }
    return jsonResponse(request, env, await pending);
  } catch (error) {
    return jsonResponse(request, env, { error: error.message }, error.message.includes('zoom in') ? 400 : 502);
  }
}
...
async function routeGrid(request, env) {
  ...
  const coverage = url.searchParams.get('coverage') === 'regional' ? 'regional' : 'bounds';
  ...
  let pending = openMeteoGridPending.get(requestKey);
  if (!pending) {
    pending = fetchOpenMeteoGrid(bounds, category, horizonHours, env, { coverage }).finally(() => {
      openMeteoGridPending.delete(requestKey);
    });
    openMeteoGridPending.set(requestKey, pending);
  }
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      if (url.pathname === '/api/regions') return routeRegions(request, env);
      if (url.pathname === '/api/forecast') return routeForecast(request, env);
      if (url.pathname === '/api/spatial') return routeSpatial(request, env);
